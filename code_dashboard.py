...
# grouped bar (gross income & sales)
with col1:
    grouped = data_selection.groupby("Product line")[["Sales", "gross income"]].sum().reset_index()
    fig = px.bar(
        grouped,
        x="Product line",
        y=["Sales", "gross income"],
        barmode="group",
        title="Sales vs Gross Income per Product Line",
...
quantity_by_product = data_selection.groupby("Product line")["Quantity"].sum().sort_values(ascending=True)
quantity_df = quantity_by_product.reset_index()
...
with col1:
    ...
    fig = px.bar(
        ...
        y="Product line",
        orientation="h",
        title="Product Line Sales Volume for Inventory Planning",
        color="Quantity",
        color_continuous_scale="purpor",
        labels={"Quantity": "Total Quantity", "Product line": "Product Line"}
        )
    fig.update_layout(
        plot_bgcolor="rgba(0,0,0,0)",  
        paper_bgcolor="rgba(0,0,0,0)", 
        title_font=dict(size=20, color="white"),
        xaxis=dict(title="<b>Total Quantity</b>", color="white"),
        yaxis=dict(title="<b>Product Line</b>", color="white"),
        coloraxis_colorbar=dict(title="Quantity")
        )
    st.plotly_chart(fig)
with col2:
    # Quantity Terjual vs Unit Price
    fig = px.scatter(
        data_selection,
        x="Unit price",
        y="Quantity",
        color="Product line",
        size="Sales",  # Optional: ukuran titik berdasarkan total transaksi
        hover_data=["Gender", "Payment"],
        color_discrete_sequence=["#815ac5", "#a9e8a1", "#9c3847", "#f8f2fc", "#fc6fd7", "#b22fcd"],
...
data_selection["Datetime"] = pd.to_datetime(
    data_selection["Date"].astype(str) + " " + data_selection["Time"].astype(str),
    format="%Y-%m-%d %I:%M:%S %p"
)
data_selection["Hour"] = data_selection["Datetime"].dt.hour
# Hitung jumlah transaksi per jam
transactions_per_hour = data_selection.groupby("Hour")["Invoice ID"].count().reset_index()
transactions_per_hour.rename(columns={"Invoice ID": "Transaction Count"}, inplace=True)
# plot
fig = px.bar(
    transactions_per_hour,
    x="Hour",
    y="Transaction Count",
    title="<b>Transactions per Hour</b>",
    color_discrete_sequence=["#d953e0"] * len(transactions_per_hour),
